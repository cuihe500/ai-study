...
## TDD与SDD的关系：两种方法论如何配合

读到这里，你可能会想：SDD 和 TDD 都是"在写代码之前先想清楚"的方法论，它们有什么区别？我该用哪个？
...
这两者不是竞争关系，而是互补关系。在一个规模中等以上的项目里，你完全可以先用 SDD 工具（比如上一节介绍的 Spec Kit 或 Kiro）建立功能规格，然后在每一个功能的实现阶段切换到 TDD 模式，用测试为 AI 导航具体的实现。

对于小规模的功能开发，如果你不需要完整的 SDD 规格，TDD 本身也是一个足够轻量的出发点——写几个测试用例，通常已经足够让你把需求想清楚了。
...
SDD 和 TDD 不是非此即彼的选择，而是在软件工程的不同层次上发挥作用：SDD 确保你在正确的路上走，TDD 确保每一步走得扎实。两者结合，构成了 AI 时代最严谨的工程实践。

下一节，我们会认识另一种方法论——SDD（规范驱动开发）。如果说 TDD 是在开始前定义好"怎么验证"，SDD 则是在开始前写清楚"要构建什么"，两者相辅相成，共同构成了 AI 编程时代最严谨的工程实践。
